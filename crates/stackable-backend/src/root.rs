...
use stackable_bridge::state::{BridgeMetadataState, BridgeState};
...
#[derive(Properties)]
pub(crate) struct StackableRootProps<CTX, BCTX> {
...
impl<CTX, BCTX> PartialEq for StackableRootProps<CTX, BCTX> {
...
impl<CTX, BCTX> Clone for StackableRootProps<CTX, BCTX> {
    fn clone(&self) -> Self {
        Self {
...
#[function_component]
fn Inner<COMP, CTX, BCTX>(props: &StackableRootProps<CTX, BCTX>) -> Html
where
...
#[function_component]
pub(crate) fn StackableRoot<COMP, CTX, BCTX>(props: &StackableRootProps<CTX, BCTX>) -> Html
where
    COMP: BaseComponent<Properties = ServerAppProps<CTX>>,
    CTX: 'static,
    BCTX: 'static,
